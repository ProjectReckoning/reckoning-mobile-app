...
export default function AccountCard({ user }) {
  const cards = [
    ...
    {
      type: "account",
      tipeTabungan: "TAPLUS BISNIS",
      noRekening: "1916554321",
      saldo: "Rp27.340.170",
    },
  ];
  const CARD_COUNT = cards.length - 1;
  ...
  const handleScroll = Animated.event(
    [{ nativeEvent: { contentOffset: { x: scrollX } } }],
    {
      ...
      listener: (event) => {
        const index = Math.round(event.nativeEvent.contentOffset.x / (288 + 8));
        ...
        >
          {cards.map((card, idx) =>
            card.type === "portfolio" ? (
              ...
              <AccountCardItem
                key={card.noRekening}
                tipeTabungan={card.tipeTabungan}
                noRekening={card.noRekening}
                saldo={card.saldo}
            ...
            return (
              <Animated.View
                ...
                style={{
                  ...
                  height: 8,
                  borderRadius: 4,
                  backgroundColor: WondrColors["dark-gray-wondr-deactive"],
                  opacity: dotOpacity,
                }}
              />
            );
